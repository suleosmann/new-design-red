...
const AmountSelector = () => {
  ...
  const handleAmountClick = (formattedAmount) => {
    ...
    if (formattedAmount !== "Other") {
      setIncludeProcessingFee(false);  // Ensure checkbox is unchecked when switching amounts
    }
  };
  ...
  return (
    <div className="max-w-lg mx-auto p-4 bg-white text-lg font-sans font-bold">
      <h1 className="text-5xl mt-24 font-bold mb-10 text-center">{donationDisplayText}</h1>
      <div className="mb-4 text-center">
        <h2 className="mb-2">How much would you like to donate?</h2>
      ...
      {donationAmount === "Other" && (
        <input
          ...
          className="w-full px-4 py-2 border-2 border-gray-300 rounded mb-4"
          value={otherAmount}
        ...
        <div className="mb-4 ml-12">
          <label className="inline-flex items-center space-x-2 font-normal">
            <input
                ...
                className="form-checkbox h-5 w-5 text-custom-red accent-custom-red"
            />
            <span>{`Add ${potentialProcessingFee} to cover processing fees & other expenses associated with my donation`}</span>
          </label>
        </div>
      )}
    </div>
